...
fn union_instanceof_field_correct_or_skip() {
    ...
    let cases: &[(&str, &str)] = &[
        ...
        ("class Cell { integer id = 5 } Cell | integer x = 3; if (x instanceof Cell) { return x.id } else { return abs(x) }", "3"),
        // instanceof false → else branch, add(10, 5) = 15.
        ("class A { integer v = 0 } A | integer x = 10; function add(integer a, integer b) { return a + b } if (x instanceof A) { return x.v } else { return add(x, 5) }", "15"),
        // instanceof TRUE (x really is a Cell) → then branch, x.id = 7.
        ("class Cell { integer id = 7 } Cell | integer x = new Cell(); if (x instanceof Cell) { return x.id } else { return abs(x) }", "7"),
    ];
