...
fn classref_as_value_constructs_or_skips() {
    ...
    let cases: &[(&str, &str, u8)] = &[
        ...
        ("class A { x constructor(x) { this.x = x } } var a = [1, 2, 3, 4] return arrayMap(a, A)",
         "[A {x: 1}, A {x: 2}, A {x: 3}, A {x: 4}]", 3),
        ("class A { x constructor(x) { this.x = x } } var a = [1, 2, 3, 4] return arrayMap(a, A)",
         "[A {x: 1}, A {x: 2}, A {x: 3}, A {x: 4}]", 2),
    ];
...
fn unconstructible_classref_value_skips() {
    ...
    let cases: &[(&str, u8)] = &[
        ("class A extends Array {} var a = [1, 2] return arrayMap(a, A)", 4),
        ("class A { x constructor(x){this.x=x} string(){return 'z'} } var a = [1] return arrayMap(a, A)", 4),
        ("class A { x constructor(x) { this.x = x } } var a = [1, 2] return arrayMap(a, A)", 1),
    ];
