...
fn reflection_field_copy_or_skip() {
    ...
    let cases: &[(&str, &str, u8)] = &[
        ...
        (
            "class A { x constructor(x) { this.x = x } } return new A()",
            "A {x: null}",
            4,
        ),
        // x.class.fields on a runtime class-ref value.
        (
            "class T { a b constructor(a,b){this.a=a this.b=b} } var t = new T(1,2) return t.class.fields",
            "[\"a\", \"b\"]",
            4,
        ),
    ];
...
fn omitted_scalar_ctor_param_skips() {
    ...
    let n = native_v(
        "class A { integer x constructor(integer x) { this.x = x } } return new A()",
        4,
    );
