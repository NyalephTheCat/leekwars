...
fn string_override_or_skip() {
    let cases: &[(&str, &str)] = &[
        ("class A { string() { return 'test' } } return new A()", "test"),
        ("class A { x = 5 string() { return 'v' + this.x } } return new A()", "v5"),
        // top-level only: a nested instance renders normally (no string()).
        ("class A { x = 5 string() { return 'v' } } return [new A()]", "[A {x: 5}]"),
        // an instance that isn't the result is unaffected.
        ("class A { string() { return 'x' } } var a = new A() return 7", "7"),
    ];
    for (i, (src, expected)) in cases.iter().enumerate() {
        let n = native_v(src, 4);
        let ok = n == *expected || n.starts_with("ERR");
        eprintln!("case {i}: native={n:?} expected={expected:?} {}", if ok { "OK" } else { "MISCOMPILE" });
