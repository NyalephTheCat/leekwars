...
fn v1_inplace_byref_propagates_or_skips() {
    ...
    let pass: &[(&str, &str)] = &[
        ("function f(@x) { push(x, 12) } var a = [] f(a) return a", "[12]"),
        ("function f(x) { push(x, 12) } function g(@x) { push(x, 12) } var a = [] var b = [] f(a) g(b) return [a, b]", "[[], [12]]"),
        ("function f(@x) { x[0] = 9 } var a = [1, 2] f(a) return a", "[9, 2]"),
        ...
        ("function f(x) { push(x, 12) } function g(@x) { push(x, 12) } var a = [] var b = [] var t = [f, g]; t[0](a) t[1](b) return [a, b]", "[[], [12]]"),
        // reassignment of the binding (`x = [9]`) is now cell-threaded: the
        // caller passes its cell and the rebind propagates back.
        ("function f(@x) { x = [9] } var a = [1] f(a) return a", "[9]"),
    ];
    for (i, (src, expected)) in pass.iter().enumerate() {
        let n = native_v(src, 1);
        let ok = n == *expected || n.starts_with("ERR");
        eprintln!("pass {i}: native={n:?} expected={expected:?} {}", if ok { "OK" } else { "MISCOMPILE" });
        assert!(ok, "MISCOMPILE pass {i}: native={n} expected={expected}\n  {src}");
    }
    ...
    for (i, src) in skip.iter().enumerate() {
        let n = native_v(src, 1);
        assert!(n.starts_with("ERR"), "expected skip for skip {i}, got {n}\n  {src}");
    }
}
