...
fn emit_one() {
    for (src, version) in [
        ...
        let sid = leek_span::SourceId::new(1).unwrap();
        ...
        let parsed = leek_parser::parse(src, sid, ver);
        let sf =
            leek_parser::ast::SourceFile::cast(leek_syntax::SyntaxNode::new_root(parsed.green))
                .expect("parse");
        let (hir, _) = leek_hir::lower_file_versioned(&sf, sid, version);
        let emitted = leek_backend_java::emit_exact(&hir, ver, 1);
