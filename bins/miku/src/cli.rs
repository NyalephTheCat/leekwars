...
#[derive(Debug, clap::Args)]
pub struct Fight {
    /// Scenario file (`.toml` or `.json`) describing the fight. Looked up
    /// under `[fight].scenarios_dir`, then the project root. If omitted, runs
    /// the manifest's `[fight].default_scenario`; without one, lists the
    /// scenarios in `[fight].scenarios_dir`.
