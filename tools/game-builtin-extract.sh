...
#   tools/game-builtin-extract.sh --check   # exit 1 if a committed artifact drifts
#   tools/game-builtin-extract.sh --write    # overwrite the committed artifacts
#
# Two artifacts are committed. `game_constants.tsv` is extracted from the
# Java source and so needs the `official-generator` submodule; the engine's
# `consts_gen.rs` is derived from that TSV and so does *not*, which keeps
# `--check` meaningful in a tree without submodules.
...
CONSTSRS="$ROOT/crates/game/leek-game-runtime/src/consts_gen.rs"
...
# The fight constants the *engine* returns, as a generated Rust module. The
# engine and the language must agree on `CELL_OBSTACLE` or an AI's
# `getCellContent(c) == CELL_OBSTACLE` silently misfires — the language side
# resolves and folds the same names from this TSV — so the engine takes its
# values from here rather than from a hand-written table. Derived from the
# committed `$CONSTOUT`, not from the Java source, so it stays checkable in a
# tree without the `official-generator` submodule.
#
# Deliberately narrow: only the `CELL_*` family is emitted. The `USE_*` family
# drifted too, but those are load-bearing return codes (see
# `crates/game/leek-game-runtime/src/state.rs`) and widening the allowlist
# would change engine behaviour as a side effect of a codegen change.
ENGINE_CONSTS=(CELL_EMPTY CELL_ENTITY CELL_OBSTACLE CELL_PLAYER)

emit_engine_consts() {
  python3 - "$CONSTOUT" "${ENGINE_CONSTS[@]}" <<'PY'
import re
import sys
...
for name in names:
    ...
    if not re.fullmatch(r"-?\d+", value):
        sys.exit(f"{name} = {value!r} in {path} is not a foldable integer")
    rows.append((name, value))

print("// GENERATED by tools/game-builtin-extract.sh --write — do not edit.")
print("//")
print("// The fight constants the engine's `getCellContent` answers with, from")
print("// `crates/core/leek-environment/game_constants.tsv` (itself extracted from")
print("// the generator's `FightConstants.java`). The language side resolves and")
print("// folds the same names from that table, so generating them here keeps the")
print("// engine's answers and the AI's constants on one source of truth.")
print("//")
print("// `CELL_PLAYER` is upstream's deprecated alias of `CELL_ENTITY`: both are 1.")
for name, value in rows:
    print(f"pub const {name}: i64 = {value};")
PY
}
...
case "${1:-}" in
  --write)
    if [[ ! -d "$GENSRC" ]]; then
      echo "official-generator submodule is not checked out — cannot regenerate $CONSTOUT" >&2
      exit 1
    fi
    mkdir -p "$(dirname "$CONSTOUT")"
    extract_constants > "$CONSTOUT"
    echo "wrote $(wc -l < "$CONSTOUT") constants to $CONSTOUT"
    emit_engine_consts > "$CONSTSRS"
    echo "wrote $CONSTSRS"
    ;;
  --check)
    # The TSV can only be re-derived with the submodule present; the engine
    # module derives from the TSV, so it is checkable either way.
    if [[ -d "$GENSRC" ]]; then
      ...
      echo "official-generator submodule absent — skipping the TSV-vs-Java check"
    fi
    if ! diff -u "$CONSTSRS" <(emit_engine_consts); then
      echo "consts_gen.rs is out of date — run tools/game-builtin-extract.sh --write" >&2
      exit 1
    fi
    echo "engine fight constants up to date"
    ;;
  *)
    extract_constants
    ;;
